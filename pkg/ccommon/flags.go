...
var (
	...
	PSource    = cli.NewParameter(FlagSource, cli.ParameterTypeString, nil, "Specific source to modify.", false)
	SourceArg  = cli.NewStringArgument("source", PSource)
	SourceFlag = cli.NewStringFlag("s", "source", PSource)

	PToolchain    = cli.NewParameter(FlagToolchain, cli.ParameterTypeString, nil, "Select toolchain to use.", false)
	ToolchainFlag = cli.NewStringFlag("T", "toolchain", PToolchain)

	PDryRun    = cli.NewParameter(FlagDryRun, cli.ParameterTypeBool, cli.PBool(false), "Show commands without executing them.", false)
	DryRunFlag = cli.NewBoolFlag("d", "dry-run", PDryRun)

	PReinit    = cli.NewParameter(FlagReinit, cli.ParameterTypeBool, cli.PBool(false), "Reinitialize the workspace.", false)
	ReinitFlag = cli.NewBoolFlag("", "reinit", PReinit)

	PDownloadDeps    = cli.NewParameter(FlagDownload, cli.ParameterTypeBool, cli.PBool(false), "Download dependencies during clone.", false)
	DownloadDepsFlag = cli.NewBoolFlag("", "download-deps", PDownloadDeps)

	PSubmodule    = cli.NewParameter(FlagSubmodule, cli.ParameterTypeBool, cli.PBool(false), "Add as a git submodule instead of cloning.", false)
	SubmoduleFlag = cli.NewBoolFlag("", "submodule", PSubmodule)

	PDelete    = cli.NewParameter(FlagDelete, cli.ParameterTypeBool, cli.PBool(false), "Delete files when removing source.", false)
	DeleteFlag = cli.NewBoolFlag("X", "delete", PDelete)

	PNoSetup    = cli.NewParameter(FlagNoSetup, cli.ParameterTypeBool, cli.PBool(false), "Don't run setup after downloading or cloning.", false)
	NoSetupFlag = cli.NewBoolFlag("", "no-setup", PNoSetup)

	PHelp    = cli.NewParameter(FlagHelp, cli.ParameterTypeBool, cli.PBool(false), "Show this help message.", false)
	HelpFlag = cli.NewBoolFlag("h", "help", PHelp)

	PDebug    = cli.NewParameter(FlagDebug, cli.ParameterTypeBool, cli.PBool(false), "Show debug information.", false)
	DebugFlag = cli.NewBoolFlag("", "debug", PDebug)

	POverwrite    = cli.NewParameter(FlagOverwrite, cli.ParameterTypeBool, cli.PBool(false), "Overwrite target if it exists.", false)
	OverwriteFlag = cli.NewBoolFlag("", "overwrite", POverwrite)

	PProjectType    = cli.NewParameter(FlagProjectType, cli.ParameterTypeString, nil, "The project type (e.g., CMake).", false)
	ProjectTypeFlag = cli.NewStringFlag("p", "project-type", PProjectType)

	PCMakePackageName    = cli.NewParameter(FlagCMakePackageName, cli.ParameterTypeString, nil, "The CMake package name.", false)
